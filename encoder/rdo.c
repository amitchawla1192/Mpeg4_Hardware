/* Completely checked for Hardware :) */
/*****************************************************************************
...
//extern uint8_t x264_cabac_transition_unary[15][128];/// Matrix to be copied to hardware
//extern uint16_t x264_cabac_size_unary[15][128];/// Matrix to be copied to hardware
/* Transition and size tables for abs>9 MVD */
/* Consist of 5 1s and a bypass sign bit */
//extern uint8_t cabac_transition_5ones[128];/// Matrix to be copied to hardware
//extern uint16_t cabac_size_5ones[128];/// Matrix to be copied to hardware
...
#define COPY_CABAC memcpy( &cabac_tmp.f8_bits_encoded, &h->cabac.f8_bits_encoded, \
...
uint64_t x264_pixel_hadamard_ac_16x16( pixel *pix, intptr_t stride );       
uint64_t x264_pixel_hadamard_ac_8x16( pixel *pix, intptr_t stride );       
uint64_t x264_pixel_hadamard_ac_16x8( pixel *pix, intptr_t stride );       
uint64_t x264_pixel_hadamard_ac_8x8( pixel *pix, intptr_t stride );       
static ALWAYS_INLINE uint64_t cached_hadamard( x264_t *h, int size, int x, int y )/// X264_MACROBLOCK_ANALYSE
{
	...
	//fprintf(stderr,"coming in that hadamard %d\n",size);
	switch(size)
	{//res = h->pixf.hadamard_ac[size]( fenc, FENC_STRIDE );
	
	 case 0:res = x264_pixel_hadamard_ac_16x16( fenc, FENC_STRIDE );
	    break;
	  case 1:res = x264_pixel_hadamard_ac_16x8( fenc, FENC_STRIDE );
	    break;
	  case 2:res = x264_pixel_hadamard_ac_8x16( fenc, FENC_STRIDE );
	    break;
	  case 3:res = x264_pixel_hadamard_ac_8x8( fenc, FENC_STRIDE );
	   break;
	}
...
static ALWAYS_INLINE int cached_satd( x264_t *h, int size, int x, int y )/// X264_MACROBLOCK_ANALYSE
{
    ...
    else
    {
        pixel *fenc = h->mb.pic.p_fenc[0] + x + y*FENC_STRIDE;
        int dc;
	//dc = h->pixf.sad[size]( fenc, FENC_STRIDE, zero, 0 ) >> 1;
      //  res = h->pixf.satd[size]( fenc, FENC_STRIDE, zero, 0 ) - dc;
	switch (size)
		   { // dc = h->pixf.sad[size]( fenc, FENC_STRIDE, zero, 0 ) >> 1;
		//	res = h->pixf.satd[size]( fenc, FENC_STRIDE, zero, 0 ) - dc;
		    	  case 0: dc = x264_pixel_sad_16x16( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_16x16( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break;  
		   	  case 1: dc = x264_pixel_sad_16x8( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_16x8( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break;
			  case 2: dc = x264_pixel_sad_8x16( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_8x16( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break; 
			  case 3: dc = x264_pixel_sad_8x8( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_8x8( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break; 
			  case 4: dc = x264_pixel_sad_8x4( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_8x4( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break; 
		 	  case 5:dc = x264_pixel_sad_4x8( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_4x8( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break;
			  case 6: dc = x264_pixel_sad_4x4( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_4x4( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break;
		          case 7: dc = x264_pixel_sad_4x16( fenc, FENC_STRIDE, zero, 0 ) >> 1;
				  res = x264_pixel_satd_4x16( fenc, FENC_STRIDE, zero, 0 ) - dc;
			    break;
		   }
...
static inline int ssd_plane( x264_t *h, int size, int p, int x, int y )/// X264_MACROBLOCK_ANALYSE
{
    ALIGNED_16( static pixel zero[16] ) = {0};
    int satd = 0,ssd_temp =0;
    ...
    if( p == 0 && h->mb.i_psy_rd )
    {
        ...
        {
            uint64_t fdec_acs;
	    //fdec_acs= h->pixf.hadamard_ac[size]( fdec, FDEC_STRIDE );
	  // fprintf(stderr,"coming in this hadamard %d\n",size);
	      switch(size)
	      {// fdec_acs = h->pixf.hadamard_ac[size]( fdec, FDEC_STRIDE );
	  
	    
		case 0:fdec_acs = x264_pixel_hadamard_ac_16x16( fdec, FDEC_STRIDE );
		  break;
		case 1:fdec_acs = x264_pixel_hadamard_ac_16x8( fdec, FDEC_STRIDE );
		  break;
		case 2:fdec_acs = x264_pixel_hadamard_ac_8x16( fdec, FDEC_STRIDE );
		  break;
		case 3:fdec_acs = x264_pixel_hadamard_ac_8x8( fdec, FDEC_STRIDE );
		  break;
	      }
        ...
        {
            int dc;
	    //dc = h->x264_pixel_sad_16x16( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
            //satd = abs(x264_pixel_satd_16x16( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
	    	switch (size)
		   { //dc = h->pixf.sad[size]( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
		      // satd = abs(h->pixf.satd[size]( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
		    	  case 0: dc = x264_pixel_sad_16x16( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_16x16( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break;  
		   	  case 1: dc = x264_pixel_sad_16x8( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_16x8( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break;
			  case 2: dc = x264_pixel_sad_8x16( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_8x16( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break; 
			  case 3: dc = x264_pixel_sad_8x8( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_8x8( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break; 
			  case 4: dc = x264_pixel_sad_8x4( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_8x4( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break; 
		 	  case 5:dc = x264_pixel_sad_4x8( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_4x8( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break;
			  case 6: dc = x264_pixel_sad_4x4( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_4x4( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break;
		          case 7: dc = x264_pixel_sad_4x16( fdec, FDEC_STRIDE, zero, 0 ) >> 1;
				  satd = abs(x264_pixel_satd_4x16( fdec, FDEC_STRIDE, zero, 0 ) - dc - cached_satd( h, size, x, y ));
			    break;
		   }
   ...
   switch (size)
		   { //ssd_temp = h->pixf.ssd[size](fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
		    	  case 0: ssd_temp = x264_pixel_ssd_16x16(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
				  break;  
		   	  case 1: ssd_temp = x264_pixel_ssd_16x8(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break;
			  case 2:ssd_temp = x264_pixel_ssd_8x16(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break; 
			  case 3: ssd_temp = x264_pixel_ssd_8x8(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break; 
			  case 4: ssd_temp = x264_pixel_ssd_8x4(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break; 
		 	  case 5: ssd_temp = x264_pixel_ssd_4x8(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break;
			  case 6:ssd_temp = x264_pixel_ssd_4x4(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break;
		          case 7: ssd_temp = x264_pixel_ssd_4x16(fenc, FENC_STRIDE, fdec, FDEC_STRIDE);
			    break;
		   }
   return ssd_temp + satd;
}

static inline int ssd_mb( x264_t *h )/// X264_MACROBLOCK_ANALYSE
{
...
static int x264_rd_cost_mb( x264_t *h, int i_lambda2 )/// X264_MACROBLOCK_ANALYSE
{
...
static uint64_t x264_rd_cost_subpart( x264_t *h, int i_lambda2, int i4, int i_pixel )/// called by X264_MACROBLOCK_ANALYSE
{
...
uint64_t x264_rd_cost_part( x264_t *h, int i_lambda2, int i4, int i_pixel )/// called by X264_MACROBLOCK_ANALYSE
{
    uint64_t i_ssd, i_bits;
    int i8 = i4 >> 2;
/*
...
static uint64_t x264_rd_cost_i8x8( x264_t *h, int i_lambda2, int i8, int i_mode, pixel edge[4][32] )/// called by X264_MACROBLOCK_ANALYSE
{
    uint64_t i_ssd, i_bits;
...
static uint64_t x264_rd_cost_i4x4( x264_t *h, int i_lambda2, int i4, int i_mode )/// called by X264_MACROBLOCK_ANALYSE
{
    uint64_t i_ssd, i_bits;
    int plane_count = CHROMA444 ? 3 : 1;
    int i_qp = h->mb.i_qp;
...
static uint64_t x264_rd_cost_chroma( x264_t *h, int i_lambda2, int i_mode, int b_dct )/// called by X64_MACROBLOCK_ANALYSE
{
    uint64_t i_ssd, i_bits;
...
static NOINLINE
int trellis_dc_shortcut( int sign_coef, int quant_coef, int unquant_mf, int coef_weight, int lambda2, uint8_t *cabac_state, int cost_sig )/// X264_MACROBLOCK_ENCODE
{
    ...
    for( int abs_level = q-1; abs_level <= q; abs_level++ )
    {
        ...
        if( abs_level )
        {
            ...
            f8_bits += x264_cabac_size_unary_hw[prefix][cabac_state[5]];
            if( abs_level >= 15 )
...
int trellis_coef( int j, int const_level, int abs_level, int prefix, int suffix_cost,
                  ...
                  trellis_level_t *level_tree, int levels_used, int lambda2, uint8_t *level_state )/// X264_MACROBLOCK_ENCODE
{
    ...
    if( const_level > 1 )
    {
        ...
        f8_bits += x264_cabac_size_unary_hw[prefix][levelgt1_state] + suffix_cost;
    }
    else
    ...
    if( score < nodes_cur[node_ctx].score )
    {
        ...
        if( const_level > 1 && node_ctx == 7 )
            nodes_cur[node_ctx].cabac_state[levelgt1_ctx-6] = x264_cabac_transition_unary_hw[prefix][levelgt1_state];
...
int trellis_coef0_0( uint64_t ssd0, trellis_node_t *nodes_cur, trellis_node_t *nodes_prev,
                     trellis_level_t *level_tree, int levels_used )/// X264_MACROBLOCK_ENCODE
{
...
int trellis_coef1_0( uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     ...
                     uint8_t *level_state )/// X264_MACROBLOCK_ENCODE
{
...
int trellis_coefn_0( int abs_level, uint64_t ssd0, uint64_t ssd1, int cost_siglast[3],
                     trellis_node_t *nodes_cur, trellis_node_t *nodes_prev,
                     trellis_level_t *level_tree, int levels_used, int lambda2,
                     uint8_t *level_state, int levelgt1_ctx )/// X264_MACROBLOCK_ENCODE
{
    int prefix = X264_MIN( abs_level-1, 14 );
...
int quant_trellis_cabac( x264_t *h, dctcoef *dct,
                         ...
                         int b_chroma, int dc, int num_coefs, int idx )/// X264_MACROBLOCK_ENCODE
{
    ...
    if( dc )
    {
        if( num_coefs == 16 )
        {
            ...
            if( !quant_4x4_dc( dct, quant_mf[0] >> 1, quant_bias[0] << 1 ) )
                return 0;
            zigzag_scan_4x4_frame( quant_coefs, dct );
        }
        else
        {
            memcpy( orig_coefs, dct, sizeof(dctcoef)*num_coefs );
            int nz = quant_2x2_dc( &dct[0], quant_mf[0] >> 1, quant_bias[0] << 1 );
            if( num_coefs == 8 )
                nz |= quant_2x2_dc( &dct[4], quant_mf[0] >> 1, quant_bias[0] << 1 );
            if( !nz )
                return 0;
    ...
    else
    {
        if( num_coefs == 64 )
        {
            memcpy( orig_coefs, dct, sizeof(dctcoef)*64 );
            if( !quant_8x8( dct, quant_mf, quant_bias ) )
                return 0;
            zigzag_scan_8x8_frame( quant_coefs, dct );
        }
        else //if( num_coefs == 16 )
        {
            memcpy( orig_coefs, dct, sizeof(dctcoef)*16 );
            if( !quant_4x4( dct, quant_mf, quant_bias ) )
                return 0;
            zigzag_scan_4x4_frame( quant_coefs, dct );
        }
    }
...
//    last_nnz = h->quantf.coeff_last[ctx_block_cat]( quant_coefs+b_ac )+b_ac;
    ...
    {
      case 0:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 1:last_nnz = x264_coeff_last15( quant_coefs+b_ac )+b_ac;
	break;
      case 2:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 3:last_nnz = x264_coeff_last4( quant_coefs+b_ac )+b_ac;
	break;
      case 4:last_nnz = x264_coeff_last15( quant_coefs+b_ac )+b_ac;
	break;
      case 5:last_nnz = x264_coeff_last64( quant_coefs+b_ac )+b_ac;
	break;
      case 6:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 7:last_nnz = x264_coeff_last15( quant_coefs+b_ac )+b_ac;
	break;
      case 8:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 9:last_nnz = x264_coeff_last64( quant_coefs+b_ac )+b_ac;
	break;
      case 10:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 11:last_nnz = x264_coeff_last15( quant_coefs+b_ac )+b_ac;
	break;
      case 12:last_nnz = x264_coeff_last16( quant_coefs+b_ac )+b_ac;
	break;
      case 13:last_nnz = x264_coeff_last64( quant_coefs+b_ac )+b_ac;
	break;
...
int quant_trellis_cavlc( x264_t *h, dctcoef *dct,
                         ...
                         const uint8_t *zigzag, int ctx_block_cat, int lambda2, int b_ac,
                         int b_chroma, int dc, int num_coefs, int idx, int b_8x8 )/// called by X264_MACROBLOCK_ENCODE
{
...
int x264_quant_luma_dc_trellis( x264_t *h, dctcoef *dct, int i_quant_cat, int i_qp, int ctx_block_cat, int b_intra, int idx )/// X264_MACROBLOCK_ENCODE
{
    if( h->param.b_cabac )
...
static const uint8_t x264_zigzag_scan2x2[4] = { 0, 1, 2, 3 };/// called by X264_MACROBLOCK_ENCODE
static const uint8_t x264_zigzag_scan2x4[8] = { 0, 2, 1, 4, 6, 3, 5, 7 };/// called by X264_MACROBLOCK_ENCODE
