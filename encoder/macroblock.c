...
static inline void zigzag_scan_2x2_dc( dctcoef level[4], dctcoef dct[4] )/// called by X264_MACROBLOCK_ENCODE
{
    ZIG(0,0,0)
    ZIG(1,0,1)
    ZIG(2,1,0)
...
static inline void zigzag_scan_2x4_dc( dctcoef level[8], dctcoef dct[8] )/// called by X264_MACROBLOCK_ENCODE
{
    level[0] = dct[0];
    level[1] = dct[2];
    level[2] = dct[1];
...
static inline void idct_dequant_2x2_dc( dctcoef dct[4], dctcoef dct4x4[4][16], int dequant_mf[6][16], int i_qp )/// called by X264_MACROBLOCK_ENCODE
{
...
static inline void idct_dequant_2x2_dconly( dctcoef dct[4], int dequant_mf[6][16], int i_qp )/// called by X264_MACROBLOCK_ENCODE
{
    IDCT_DEQUANT_2X2_START
    dct[0] = (d0 + d1) * dmf >> 5;
    dct[1] = (d0 - d1) * dmf >> 5;
...
static inline void dct2x2dc( dctcoef d[4], dctcoef dct4x4[4][16] )/// called by X264_MACROBLOCK_ENCODE
{
...
static ALWAYS_INLINE int array_non_zero( dctcoef *v, int i_count )/// called by X264_MACROBLOCK_ENCODE
{
    if( WORD_SIZE == 8 )
    {
...
static void x264_mb_encode_i16x16( x264_t *h, int p, int i_qp )/// called by X264_MACROBLOCK_ENCODE
{
    ...
    {//h->predict_16x16[i_mode]( h->mb.pic.p_fdec[p] );
      ...
      switch ( i_mode ) {
	case 0:x264_predict_16x16_v_c( h->mb.pic.p_fdec[p] );
	  break;
	case 1:
	    x264_predict_16x16_h_c( h->mb.pic.p_fdec[p] );  
	  break;
	case 2:
	    x264_predict_16x16_dc_c( h->mb.pic.p_fdec[p] ); 
	  break;
	case 3:
	    x264_predict_16x16_p_c( h->mb.pic.p_fdec[p] );
	  break;
	case 4:
	    x264_predict_16x16_dc_left_c( h->mb.pic.p_fdec[p] );
	  break;
	case 5:
	     x264_predict_16x16_dc_top_c( h->mb.pic.p_fdec[p] );
	  break;
	case 6: 
	     x264_predict_16x16_dc_128_c( h->mb.pic.p_fdec[p] );
	  break;
      }
    ...
    if( h->mb.b_lossless )
    {
        for( int i = 0; i < 16; i++ )
        {
            ...
            nz = zigzag_sub_4x4ac_frame( h->dct.luma4x4[16*p+i], p_src+oe, p_dst+od, &dct_dc4x4[block_idx_yx_1d[i]] );
        ...
        zigzag_scan_4x4_frame( h->dct.luma16x16_dc[p], dct_dc4x4 );
        return;
    }
    ...
    if( h->mb.b_noise_reduction )
        for( int idx = 0; idx < 16; idx++ )
            x264_denoise_dct( dct4x4[idx], h->nr_residual_sum[0], h->nr_offset[0], 16 );
    ...
    if( h->mb.b_trellis )
    {
        for( int idx = 0; idx < 16; idx++ )
            ...
            {
                block_cbp = 0xf;
                zigzag_scan_4x4_frame( h->dct.luma4x4[16*p+idx], dct4x4[idx] );
                dequant_4x4( dct4x4[idx], h->dequant4_mf[i_quant_cat], i_qp );
                if( decimate_score < 6 ) decimate_score += x264_decimate_score15( h->dct.luma4x4[16*p+idx] );
    ...
    else
    {
        ...
        {
            nz = quant_4x4x4( &dct4x4[i8x8*4], h->quant4_mf[i_quant_cat][i_qp], h->quant4_bias[i_quant_cat][i_qp] );
            if( nz )
            {
                block_cbp = 0xf;
    ...
    if( h->mb.b_trellis )
        ...
        nz = quant_4x4_dc( dct_dc4x4, h->quant4_mf[i_quant_cat][i_qp][0]>>1, h->quant4_bias[i_quant_cat][i_qp][0]<<1 );
    ...
    if( nz )
    {
        ...
        idct4x4dc( dct_dc4x4 );
        dequant_4x4_dc( dct_dc4x4, h->dequant4_mf[i_quant_cat], i_qp );  /* XXX not inversed */
        if( block_cbp )
            for( int i = 0; i < 16; i++ )
...
static ALWAYS_INLINE int x264_mb_optimize_chroma_dc( x264_t *h, dctcoef *dct_dc, int dequant_mf[6][16], int i_qp, int chroma422 )/// called by X264_MACROBLOCK_ENCODE
{
    int dmf = dequant_mf[i_qp%6][0] << i_qp/6;
    ...
    if( chroma422 )
        return optimize_chroma_2x4_dc( dct_dc, dmf );
    else
        return optimize_chroma_2x2_dc( dct_dc, dmf );
}

static ALWAYS_INLINE void x264_mb_encode_chroma_internal( x264_t *h, int b_inter, int i_qp, int chroma422 )/// called by X264_MACROBLOCK_ENCODE
{
    int nz, nz_dc;
    ...
    if( b_decimate && i_qp >= (h->mb.b_trellis ? 12 : 18) && !h->mb.b_noise_reduction )
    {
        ...
        int score;
	//score = h->pixf.var2[chromapix]( h->mb.pic.p_fenc[1], FENC_STRIDE, h->mb.pic.p_fdec[1], FDEC_STRIDE, &ssd[0] );
	switch(chromapix)
	{
	  case 2: score = x264_pixel_var2_8x16( h->mb.pic.p_fenc[1], FENC_STRIDE, h->mb.pic.p_fdec[1], FDEC_STRIDE, &ssd[0] );
	    break;
	  case 3: score = x264_pixel_var2_8x8( h->mb.pic.p_fenc[1], FENC_STRIDE, h->mb.pic.p_fdec[1], FDEC_STRIDE, &ssd[0] );
	    break;
	}
        if( score < thresh*4 )
	{   //score += h->pixf.var2[chromapix]( h->mb.pic.p_fenc[2], FENC_STRIDE, h->mb.pic.p_fdec[2], FDEC_STRIDE, &ssd[1] );
	  switch(chromapix)
	{
	  case 2: score += x264_pixel_var2_8x16( h->mb.pic.p_fenc[2], FENC_STRIDE, h->mb.pic.p_fdec[2], FDEC_STRIDE, &ssd[1] );
	    break;
	  case 3: score += x264_pixel_var2_8x8( h->mb.pic.p_fenc[2], FENC_STRIDE, h->mb.pic.p_fdec[2], FDEC_STRIDE, &ssd[1] );
	    break;
	}
	}
        if( score < thresh*4 )
            ...
            for( int ch = 0; ch < 2; ch++ )
            {
                if( ssd[ch] > thresh )
                {
                    ...
                    if( chroma422 )
                        ...
                        sub8x16_dct_dc( dct_dc, p_src, p_dst );
                    else
                        sub8x8_dct_dc( dct_dc, p_src, p_dst );
                    ...
                    else
                    {
                        ...
                        for( int i = 0; i <= chroma422; i++ )
                            nz_dc |= quant_2x2_dc( &dct_dc[4*i], h->quant4_mf[CQM_4IC+b_inter][i_qp+3*chroma422][0] >> 1,
    ...
    for( int ch = 0; ch < 2; ch++ )
    {
        ...
        if( h->mb.b_lossless )
        {
            ...
            for( int i = 0; i < (chroma422?8:4); i++ )
            {
                ...
                nz = zigzag_sub_4x4ac_frame( h->dct.luma4x4[16+i+(chroma422?i&4:0)+ch*16], p_src+oe, p_dst+od,
        ...
        for( int i = 0; i <= chroma422; i++ )
            sub8x8_dct( &dct4x4[4*i], p_src + 8*i*FENC_STRIDE, p_dst + 8*i*FDEC_STRIDE );

        if( h->mb.b_noise_reduction )
            for( int i = 0; i < (chroma422?8:4); i++ )
                x264_denoise_dct( dct4x4[i], h->nr_residual_sum[2], h->nr_offset[2], 16 );
        ...
        for( int i8x8 = 0; i8x8 < (chroma422?2:1); i8x8++ )
        {
            if( h->mb.b_trellis )
            {
                for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                {
                    if( x264_quant_4x4_trellis( h, dct4x4[i8x8*4+i4x4], CQM_4IC+b_inter, i_qp, DCT_CHROMA_AC, !b_inter, 1, 0 ) )
                    {
                        ...
                        zigzag_scan_4x4_frame( h->dct.luma4x4[idx], dct4x4[i8x8*4+i4x4] );
                        dequant_4x4( dct4x4[i8x8*4+i4x4], dequant_mf, i_qp );
                        if( i_decimate_score < 7 )
                            i_decimate_score += x264_decimate_score15( h->dct.luma4x4[idx] );
            ...
            {
                nz = quant_4x4x4( &dct4x4[i8x8*4], h->quant4_mf[CQM_4IC+b_inter][i_qp],
        ...
        else
        {
            ...
            if( nz_dc )
            {
                ...
                {
                    zigzag_scan_2x4_dc( h->dct.chroma_dc[ch], dct_dc );
                    idct_dequant_2x4_dc( dct_dc, dct4x4, dequant_mf, i_qp+3 );
                }
                else
                {
                    zigzag_scan_2x2_dc( h->dct.chroma_dc[ch], dct_dc );
...
void x264_mb_encode_chroma( x264_t *h, int b_inter, int i_qp )/// called by X264_MACROBLOCK_ENCODE
{
...
void x264_predict_lossless_chroma( x264_t *h, int i_mode )/// called by X264_MACROBLOCK_ENCODE
{
    int height = 16 >> CHROMA_V_SHIFT;
    if( i_mode == I_PRED_CHROMA_V )
    {
        mc_copy_w8( h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1]-FENC_STRIDE, FENC_STRIDE, height );
        mc_copy_w8( h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2]-FENC_STRIDE, FENC_STRIDE, height );
    ...
    {
        mc_copy_w8( h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1]-1, FENC_STRIDE, height );
        mc_copy_w8( h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2]-1, FENC_STRIDE, height );
...
void x264_predict_lossless_4x4( x264_t *h, pixel *p_dst, int p, int idx, int i_mode )/// called by X264_MACROBLOCK_ANALYSE
{
    ...
    if( i_mode == I_PRED_4x4_V )
        mc_copy_w4( p_dst, FDEC_STRIDE, p_src-stride, stride, 4 );
    else if( i_mode == I_PRED_4x4_H )
        mc_copy_w4( p_dst, FDEC_STRIDE, p_src-1, stride, 4 );
    else
...
void x264_predict_lossless_8x8( x264_t *h, pixel *p_dst, int p, int idx, int i_mode, pixel edge[36] )/// called by X264_MACROBLOCK_ANALYSE
{
    int stride = h->fenc->i_stride[p] << MB_INTERLACED;
    pixel *p_src = h->mb.pic.p_fenc_plane[p] + (idx&1)*8 + (idx>>1)*8*stride;

    if( i_mode == I_PRED_8x8_V )
        mc_copy_w8( p_dst, FDEC_STRIDE, p_src-stride, stride, 8 );
    else if( i_mode == I_PRED_8x8_H )
        mc_copy_w8( p_dst, FDEC_STRIDE, p_src-1, stride, 8 );
    else
...
void x264_predict_lossless_16x16( x264_t *h, int p, int i_mode )/// called by X264_MACROBLOCK_ANALYSE
{
    int stride = h->fenc->i_stride[p] << MB_INTERLACED;
    if( i_mode == I_PRED_16x16_V )
        mc_copy_w16( h->mb.pic.p_fdec[p], FDEC_STRIDE, h->mb.pic.p_fenc_plane[p]-stride, stride, 16 );
    else if( i_mode == I_PRED_16x16_H )
        mc_copy_w16( h->mb.pic.p_fdec[p], FDEC_STRIDE, h->mb.pic.p_fenc_plane[p]-1, stride, 16 );
    else
     {
       //h->predict_16x16[i_mode]( h->mb.pic.p_fdec[p] );
     switch ( i_mode ) {
 ...
 *****************************************************************************/
static ALWAYS_INLINE void x264_macroblock_encode_internal( x264_t *h, int plane_count, int chroma )/// called by X264_MACROBLOCK_ENCODE
{
    int i_qp = h->mb.i_qp;
    ...
    if( h->mb.i_type == I_PCM )
    {
        ...
        for( int p = 0; p < plane_count; p++ )
            mc_copy_w16( h->mb.pic.p_fdec[p], FDEC_STRIDE, h->mb.pic.p_fenc[p], FENC_STRIDE, 16 );
        if( chroma )
        {
            int height = 16 >> CHROMA_V_SHIFT;
            mc_copy_w8  ( h->mb.pic.p_fdec[1], FDEC_STRIDE, h->mb.pic.p_fenc[1], FENC_STRIDE, height );
            mc_copy_w8  ( h->mb.pic.p_fdec[2], FDEC_STRIDE, h->mb.pic.p_fenc[2], FENC_STRIDE, height );
        }
        return;
    }
    ...
    if( h->mb.i_type == P_SKIP )
    {
        ...
        if( !h->mb.b_skip_mc )
        {
            ...
            for( int p = 0; p < plane_count; p++ )
                mc_luma( h->mb.pic.p_fdec[p], FDEC_STRIDE,
            ...
            if( chroma )
            {
                ...
                if( mvx | mvy )
                    mc_chroma( h->mb.pic.p_fdec[1], h->mb.pic.p_fdec[2], FDEC_STRIDE,
                                     h->mb.pic.p_fref[0][0][4], h->mb.pic.i_stride[1],
                                     mvx, 2*mvy>>v_shift, 8, height );
                else
                    load_deinterleave_chroma_fdec( h->mb.pic.p_fdec[1], h->mb.pic.p_fref[0][0][4],
                                                         h->mb.pic.i_stride[1], height );

                if( h->sh.weight[0][1].weightfn )
                    mc_weight_w8( h->mb.pic.p_fdec[1], FDEC_STRIDE,
                                                       h->mb.pic.p_fdec[1], FDEC_STRIDE,
                                                       &h->sh.weight[0][1], height );
                if( h->sh.weight[0][2].weightfn )
                    mc_weight_w8( h->mb.pic.p_fdec[2], FDEC_STRIDE,
    ...
    else if( h->mb.i_type == I_8x8 )
    {
        ...
        {
            mc_copy_w16( h->mb.pic.p_fdec[0], FDEC_STRIDE, h->mb.pic.i8x8_fdec_buf, 16, 16 );
            ...
            if( h->mb.i_skip_intra == 2 )
                memcpy( h->dct.luma8x8, h->mb.pic.i8x8_dct_buf, sizeof(h->mb.pic.i8x8_dct_buf) );
        }
        for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
        {
    ...
    else if( h->mb.i_type == I_4x4 )
    {
        ...
        if( h->mb.i_skip_intra )
        {
            mc_copy_w16( h->mb.pic.p_fdec[0], FDEC_STRIDE, h->mb.pic.i4x4_fdec_buf, 16, 16 );
            ...
            /* In RD mode, restore the now-overwritten DCT data. */
            if( h->mb.i_skip_intra == 2 )
                memcpy( h->dct.luma4x4, h->mb.pic.i4x4_dct_buf, sizeof(h->mb.pic.i4x4_dct_buf) );
        }
        for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
        {
    ...
    else    /* Inter MB */
    {
            ...
            if( h->mb.b_transform_8x8 )
                for( int p = 0; p < plane_count; p++ )
                    for( int i8x8 = 0; i8x8 < 4; i8x8++ )
                    {
                        int x = i8x8&1;
                        int y = i8x8>>1;
                        nz = zigzag_sub_8x8_frame( h->dct.luma8x8[p*4+i8x8], h->mb.pic.p_fenc[p] + 8*x + 8*y*FENC_STRIDE,
                ...
                for( int p = 0; p < plane_count; p++ )
                    ...
                    {
                        nz = zigzag_sub_4x4_frame( h->dct.luma4x4[p*16+i4x4],
        ...
        else if( h->mb.b_transform_8x8 )
        {
            ...
            {
                CLEAR_16x16_NNZ( p );
                sub16x16_dct8( dct8x8, h->mb.pic.p_fenc[p], h->mb.pic.p_fdec[p] );
                ...
                for( int idx = 0; idx < 4; idx++ )
                {
                    ...
                    if( nz )
                    {
                        zigzag_scan_8x8_frame( h->dct.luma8x8[p*4+idx], dct8x8[idx] );
                        if( b_decimate )
                        {
                            int i_decimate_8x8 = x264_decimate_score64( h->dct.luma8x8[p*4+idx] );
                ...
                if( i_decimate_mb >= 6 || !b_decimate )
                {
                    ...
                    {
                        dequant_8x8( dct8x8[idx], h->dequant8_mf[p?CQM_8PC:CQM_8PY], i_qp );
                        add8x8_idct8( &h->mb.pic.p_fdec[p][8*(idx&1) + 8*(idx>>1)*FDEC_STRIDE], dct8x8[idx] );
        ...
        else
        {
            ...
            {
                CLEAR_16x16_NNZ( p );
                sub16x16_dct( dct4x4, h->mb.pic.p_fenc[p], h->mb.pic.p_fdec[p] );

                if( h->mb.b_noise_reduction )
                {
                    h->nr_count[0+!!p*2] += 16;
                    for( int idx = 0; idx < 16; idx++ )
                        x264_denoise_dct( dct4x4[idx], h->nr_residual_sum[0+!!p*2], h->nr_offset[0+!!p*2], 16 );
                }
                ...
                for( int i8x8 = 0; i8x8 < 4; i8x8++ )
                {
                    ...
                    if( h->mb.b_trellis )
                    {
                        for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                        {
                            ...
                            {
                                zigzag_scan_4x4_frame( h->dct.luma4x4[p*16+idx], dct4x4[idx] );
                                dequant_4x4( dct4x4[idx], h->dequant4_mf[p?CQM_4PC:CQM_4PY], i_qp );
                                if( i_decimate_8x8 < 6 )
                                    i_decimate_8x8 += x264_decimate_score16( h->dct.luma4x4[p*16+idx] );
                    ...
                    {
                        nnz8x8 = nz = quant_4x4x4( &dct4x4[i8x8*4], h->quant4_mf[CQM_4PY][i_qp], h->quant4_bias[CQM_4PY][i_qp] );
                        if( nz )
                        {
                            FOREACH_BIT( idx, i8x8*4, nz )
                            {
                ...
                else
                {
                    ...
                    {
                        add8x8_idct( &h->mb.pic.p_fdec[p][(i8x8&1)*8 + (i8x8>>1)*8*FDEC_STRIDE], &dct4x4[i8x8*4] );
                    }
                }
            }
        }
...
void x264_macroblock_encode( x264_t *h ) /// called by X264_SLICE_WRITE
{
    if( CHROMA444 )
 ...
 *****************************************************************************/
static ALWAYS_INLINE int x264_macroblock_probe_skip_internal( x264_t *h, int b_bidir, int plane_count, int chroma )/// called by X264_MACROBLOCK_ANALYSE
{
    ALIGNED_ARRAY_N( dctcoef, dct4x4,[8],[16] );
    ...
    for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
    {
        ...
        for( int i8x8 = 0, i_decimate_mb = 0; i8x8 < 4; i8x8++ )
        {
            ...
            sub8x8_dct( dct4x4, h->mb.pic.p_fenc[p] + fenc_offset,
                                        h->mb.pic.p_fdec[p] + fdec_offset );

            if( h->mb.b_noise_reduction )
                for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                    x264_denoise_dct( dct4x4[i4x4], h->nr_residual_sum[0+!!p*2], h->nr_offset[0+!!p*2], 16 );

            int nz = quant_4x4x4( dct4x4, h->quant4_mf[quant_cat][i_qp], h->quant4_bias[quant_cat][i_qp] );
            FOREACH_BIT( idx, 0, nz )
            {
                zigzag_scan_4x4_frame( dctscan, dct4x4[idx] );
                i_decimate_mb += x264_decimate_score16( dctscan );
                if( i_decimate_mb >= 6 )
                    return 0;
            }
        }
    ...
    if( chroma == CHROMA_420 || chroma == CHROMA_422 )
    {
        ...
        for( int ch = 0; ch < 2; ch++ )
        {
            ...
            if( !b_bidir && h->sh.weight[0][1+ch].weightfn )
                mc_weight_w8( h->mb.pic.p_fdec[1+ch], FDEC_STRIDE,
	    ...
	    switch(chroma422)
	    {case 0:ssd = x264_pixel_ssd_8x8( p_dst, FDEC_STRIDE, p_src, FENC_STRIDE );
	      break;
	     case 1: ssd = x264_pixel_ssd_8x16( p_dst, FDEC_STRIDE, p_src, FENC_STRIDE );
		break;
            ...
            if( h->mb.b_noise_reduction )
            {
                ...
                {
                    x264_denoise_dct( dct4x4[i4x4], h->nr_residual_sum[2], h->nr_offset[2], 16 );
            ...
            for( int i = 0; i <= chroma422; i++ )
                if( quant_2x2_dc( &dct_dc[4*i], h->quant4_mf[CQM_4PC][i_qp+3*chroma422][0] >> 1,
            ...
            {
                int nz = quant_4x4x4( &dct4x4[i8x8*4], h->quant4_mf[CQM_4PC][i_qp], h->quant4_bias[CQM_4PC][i_qp] );
                FOREACH_BIT( idx, i8x8*4, nz )
                {
                   zigzag_scan_4x4_frame( dctscan, dct4x4[idx] );
                    i_decimate_mb += x264_decimate_score15( dctscan );
                    if( i_decimate_mb >= 7 )
                        return 0;
                }
            }
...
int x264_macroblock_probe_skip( x264_t *h, int b_bidir )/// called by X264_MACROBLOCK_ANALYSE
{
 ...
 *****************************************************************************/
static ALWAYS_INLINE void x264_macroblock_encode_p8x8_internal( x264_t *h, int i8, int plane_count, int chroma )/// called by X264_MACROBLOCK_ANALYSE
{
    int b_decimate = h->mb.b_dct_decimate;
    int i_qp = h->mb.i_qp;
    int x = i8&1;
    ...
    if( h->mb.b_lossless )
    {
        for( int p = 0; p < plane_count; p++ )
        {
            ...
            if( h->mb.b_transform_8x8 )
            {
                nnz8x8 = zigzag_sub_8x8_frame( h->dct.luma8x8[4*p+i8], p_fenc, p_fdec );
            ...
            else
            {
                ...
                {
                    nz = zigzag_sub_4x4_frame( h->dct.luma4x4[16*p+i4],
        ...
        if( chroma == CHROMA_420 || chroma == CHROMA_422 )
        {
            for( int ch = 0; ch < 2; ch++ )
            {
                ...
                for( int i4x4 = 0; i4x4 <= chroma422; i4x4++ )
                {
                    ...
                    nz = zigzag_sub_4x4ac_frame( h->dct.luma4x4[16+offset+ch*16], p_fenc+4*i4x4*FENC_STRIDE, p_fdec+4*i4x4*FDEC_STRIDE, &dc );
    ...
    else
    {
        if( h->mb.b_transform_8x8 )
        {
            for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
            {
                ...
                sub8x8_dct8( dct8x8, p_fenc, p_fdec );
                int nnz8x8 = x264_quant_8x8( h, dct8x8, i_qp, ctx_cat_plane[DCT_LUMA_8x8][p], 0, p, i8 );
                if( nnz8x8 )
                {
                    zigzag_scan_8x8_frame( h->dct.luma8x8[4*p+i8], dct8x8 );

                    if( b_decimate && !h->mb.b_trellis )
                        nnz8x8 = 4 <= x264_decimate_score64( h->dct.luma8x8[4*p+i8] );

                    if( nnz8x8 )
                    {
                        dequant_8x8( dct8x8, h->dequant8_mf[quant_cat], i_qp );
                        add8x8_idct8( p_fdec, dct8x8 );
        ...
        else
        {
            for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
            {
                ...
                sub8x8_dct( dct4x4, p_fenc, p_fdec );
                STORE_8x8_NNZ( p, i8, 0 );
                ...
                if( h->mb.b_trellis )
                {
                    for( int i4x4 = 0; i4x4 < 4; i4x4++ )
                    {
                        ...
                        {
                            zigzag_scan_4x4_frame( h->dct.luma4x4[p*16+i8*4+i4x4], dct4x4[i4x4] );
                            dequant_4x4( dct4x4[i4x4], h->dequant4_mf[quant_cat], i_qp );
                            if( i_decimate_8x8 < 4 )
                                i_decimate_8x8 += x264_decimate_score16( h->dct.luma4x4[p*16+i8*4+i4x4] );
                ...
                {
                    nnz8x8 = nz = quant_4x4x4( dct4x4, h->quant4_mf[quant_cat][i_qp], h->quant4_bias[quant_cat][i_qp] );
                    if( nz )
                    {
                        FOREACH_BIT( i4x4, 0, nz )
                        {
        ...
        if( chroma == CHROMA_420 || chroma == CHROMA_422 )
        {
            ...
            for( int ch = 0; ch < 2; ch++ )
            {
                ...
                for( int i4x4 = 0; i4x4 <= chroma422; i4x4++ )
                {
                    sub4x4_dct( dct4x4[i4x4], p_fenc + 4*i4x4*FENC_STRIDE, p_fdec + 4*i4x4*FDEC_STRIDE );
                    ...
                    if( h->mb.b_trellis )
                        ...
                        nz = quant_4x4( dct4x4[i4x4], h->quant4_mf[CQM_4PC][i_qp], h->quant4_bias[CQM_4PC][i_qp] );

                    int offset = chroma422 ? ((5*i8) & 0x09) + 2*i4x4 : i8;
                    h->mb.cache.non_zero_count[x264_scan8[16+offset+ch*16]] = nz;
                    if( nz )
                    {
                        zigzag_scan_4x4_frame( h->dct.luma4x4[16+offset+ch*16], dct4x4[i4x4] );
                        dequant_4x4( dct4x4[i4x4], h->dequant4_mf[CQM_4PC], i_qp );
                        add4x4_idct( p_fdec + 4*i4x4*FDEC_STRIDE, dct4x4[i4x4] );
                    }
                }
            }
            h->mb.i_cbp_chroma = 0x02;
...
void x264_macroblock_encode_p8x8( x264_t *h, int i8 )/// called by X264_MACROBLOCK_ANALYSE
{
    if( CHROMA444 )
 ...
 *****************************************************************************/
static ALWAYS_INLINE void x264_macroblock_encode_p4x4_internal( x264_t *h, int i4, int plane_count )/// called by X264_MACROBLOCK_ANALYSE
{
    int i_qp = h->mb.i_qp;
    ...
    for( int p = 0; p < plane_count; p++, i_qp = h->mb.i_chroma_qp )
    {
        ...
        if( h->mb.b_lossless )
        {
            nz = zigzag_sub_4x4_frame( h->dct.luma4x4[p*16+i4], p_fenc, p_fdec );
            h->mb.cache.non_zero_count[x264_scan8[p*16+i4]] = nz;
        }
        else
        {
            ALIGNED_ARRAY_N( dctcoef, dct4x4,[16] );
            sub4x4_dct( dct4x4, p_fenc, p_fdec );
            nz = x264_quant_4x4( h, dct4x4, i_qp, ctx_cat_plane[DCT_LUMA_4x4][p], 0, p, i4 );
            h->mb.cache.non_zero_count[x264_scan8[p*16+i4]] = nz;
            if( nz )
            {
                zigzag_scan_4x4_frame( h->dct.luma4x4[p*16+i4], dct4x4 );
                dequant_4x4( dct4x4, h->dequant4_mf[quant_cat], i_qp );
                add4x4_idct( p_fdec, dct4x4 );
            }
        }
    }
}

void x264_macroblock_encode_p4x4( x264_t *h, int i8 )/// called by X264_MACROBLOCK_ANALYSE
{
    if( CHROMA444 )
