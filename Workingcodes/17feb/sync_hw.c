...
#define COPY_DERIALISE(x,y) memcpy(x , Buff0 + derialize_pointer ,y); \
 fprintf(stdout,"copied data bytes %d and %d from address %p to %p \n",y,derialize_pointer,x,(intptr_t)x + y);  \
derialize_pointer += y;   \
			if(derialize_pointer == recv_size)\
			{ fprintf(stdout,"derializing complete");   \
			}
...
char Buff0[9 * 1024 *1024 ];

/*********   SYNC WITH SOFTWARE  *******/
//////////////////////////////////////////////////   This function is called first time to create new structure and copy matrixes at hardware side /////
//x264_t *h_hw;
x264_t *x264_hw_sync_open(x264_t **ul)
{uint32_t recv_size;
...
//COPY_DERIALISE(framesunused0,172)
 h_hw->frames.unused[0]= framesunused0;
//COPY_DERIALISE(framesunused1,84)
 h_hw->frames.unused[1]= framesunused1;
//COPY_DERIALISE(framescurrent,28) 
h_hw->frames.current = framescurrent;
//COPY_DERIALISE(framesblank_unused,16)
 h_hw->frames.blank_unused= framesblank_unused;
...
//COPY_DERIALISE(nal_buffer,1500068)
 h_hw->nal_buffer = nal_buffer;
//COPY_DERIALISE(reconfig_h,50432) 
h_hw->reconfig_h = reconfig_h;
//COPY_DERIALISE(p_bitstream,1000000)
 h_hw->thread[0]->out.p_bitstream = p_bitstream;
...
//COPY_DERIALISE(intra_border_backup00,208)
 h_hw->intra_border_backup[0][0] = intra_border_backup00; 
//COPY_DERIALISE(intra_border_backup01,208)
 h_hw->intra_border_backup[0][1] = intra_border_backup01;
//COPY_DERIALISE(intra_border_backup10,208)
 h_hw->intra_border_backup[1][0] = intra_border_backup10;
//COPY_DERIALISE(intra_border_backup11,208)
 h_hw->intra_border_backup[1][1] = intra_border_backup11;
//COPY_DERIALISE(deblock_strength0,704)
 h_hw->deblock_strength[0] = deblock_strength0;
//COPY_DERIALISE(scratch_buffer,512)
 h_hw->scratch_buffer = scratch_buffer;
//COPY_DERIALISE(scratch_buffer2,384)
 h_hw->scratch_buffer2 = scratch_buffer2;
//COPY_DERIALISE(rc,656)
 h_hw->rc = rc;
//COPY_DERIALISE(rc_pred,100)
 h_hw->rc->pred = rc_pred;
//COPY_DERIALISE(rc_pred_b_from_p,20)
h_hw->rc->pred_b_from_p = rc_pred_b_from_p;
h_hw->fenc = fenc;
h_hw->fdec = fdec;
...
////////////////// DATA INITIALIZATION ////
static int prev_out_hw_bitstream;
uint32_t recv_size[15]={0};
char Buff0[512*1024];
x264_t *h_hw=*ul;
uint16_t *quant4_mf0;
...
//COPY_DERIALISE(nal_buffer,1500068) h_hw->nal_buffer = nal_buffer;
COPY_DERIALISE(reconfig_h,50432) h_hw->reconfig_h = reconfig_h;
//COPY_DERIALISE(&prev_out_hw_bitstream,sizeof(int))
COPY_DERIALISE(p_bitstream + prev_out_hw_bitstream,h_hw->thread[0]->out.i_bitstream - prev_out_hw_bitstream) h_hw->thread[0]->out.p_bitstream = p_bitstream; prev_out_hw_bitstream = h->thread[0]->out.i_bitstream ;
