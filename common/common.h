...
#define FIX8(f) ((int)(f*(1<<8)+.5))// used in dct.c
...
#if HAVE_INTERLACED/// This one goes
...
//void x264_cavlc_init( x264_t *h );	may be decided to call in hardware
//void x264_cabac_init( x264_t *h );   may be decided to call in hardware

static ALWAYS_INLINE pixel x264_clip_pixel( int x )	// H/W and S/W
{
    return ( (x & ~PIXEL_MAX) ? (-x)>>31 & PIXEL_MAX : x );
}

static ALWAYS_INLINE int x264_clip3( int v, int i_min, int i_max )// H/W and S/W
{
    return ( (v < i_min) ? i_min : (v > i_max) ? i_max : v );
}

static ALWAYS_INLINE double x264_clip3f( double v, double f_min, double f_max )	// H/W and S/W
{
    return ( (v < f_min) ? f_min : (v > f_max) ? f_max : v );
}

static ALWAYS_INLINE int x264_median( int a, int b, int c )// H/W and S/W
{
    int t = (a-b)&((a-b)>>31);
    a -= t;
    b += t;
...
static ALWAYS_INLINE void x264_median_mv( int16_t *dst, int16_t *a, int16_t *b, int16_t *c )// H/W and S/W
{
    dst[0] = x264_median( a[0], b[0], c[0] );
    dst[1] = x264_median( a[1], b[1], c[1] );
}

static ALWAYS_INLINE int x264_predictor_difference( int16_t (*mvc)[2], intptr_t i_mvc ) // H/W
{
    int sum = 0;
...
static ALWAYS_INLINE uint16_t x264_cabac_mvd_sum( uint8_t *mvdleft, uint8_t *mvdtop ) // H/W
{
...
static ALWAYS_INLINE float x264_log2( uint32_t x )	//H/W and S/W
{
    int lz = x264_clz( x );
...
typedef struct x264_lookahead_t	// H/W and S/W
{
...
typedef struct x264_ratecontrol_t	// H/W and S/W
{
    /* constants */
    int b_abr;
    int b_2pass;
...
typedef struct x264_left_table_t	// H/W and S/W
{
    uint8_t intra[4];
    uint8_t nnz[4];
...
static int ALWAYS_INLINE x264_predictor_roundclip( int16_t (*dst)[2], int16_t (*mvc)[2], int i_mvc, int16_t mv_limit[2][2], uint32_t pmv )//H/W
{
    int cnt = 0;
...
static int ALWAYS_INLINE x264_predictor_clip( int16_t (*dst)[2], int16_t (*mvc)[2], int i_mvc, int16_t mv_limit[2][2], uint32_t pmv )// H/W
{
    int cnt = 0;
