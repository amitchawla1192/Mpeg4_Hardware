 ...
 void dct2x4dc( dctcoef dct[8], dctcoef dct4x4[8][16] )
{
 ...
 inline void pixel_sub_wxh( dctcoef *diff, int i_size,
 ...
 void sub16x16_dct( dctcoef dct[16][16], pixel *pix1, pixel *pix2 )
{
 ...
 int sub4x4_dct_dc( pixel *pix1, pixel *pix2 )
{
    int sum = 0;
...
#undef DST
 ...
 void add16x16_idct8( pixel *dst, dctcoef dct[4][64] )/// nowhere
{
 ...
 void inline add4x4_idct_dc( pixel *p_dst, dctcoef dc )
{
    dc = (dc + 32) >> 6;
...
{
    dctf->sub4x4_dct    = sub4x4_dct;// H/W in macroblock.c
    dctf->add4x4_idct   = add4x4_idct;// H/W in macroblock.c

    dctf->sub8x8_dct    = sub8x8_dct;// H/W in macroblock.c 
    dctf->sub8x8_dct_dc = sub8x8_dct_dc;// H/W in macroblock.c 
    dctf->add8x8_idct   = add8x8_idct;// H/W in macroblock.c 
    dctf->add8x8_idct_dc = add8x8_idct_dc;// H/W in macroblock.c 

    dctf->sub8x16_dct_dc = sub8x16_dct_dc;// H/W in macroblock.c 

    dctf->sub16x16_dct  = sub16x16_dct;// H/W in macroblock.c and analyse.c
    dctf->add16x16_idct = add16x16_idct;// H/W in macroblock.c 
    dctf->add16x16_idct_dc = add16x16_idct_dc;// H/W in macroblock.c 

    dctf->sub8x8_dct8   = sub8x8_dct8;// H/W in macroblock.c 
    dctf->add8x8_idct8  = add8x8_idct8;// H/W in macroblock.c 

    dctf->sub16x16_dct8  = sub16x16_dct8;// H/W in macroblock.c and analyse.c
    dctf->add16x16_idct8 = add16x16_idct8;// nowhere

    dctf->dct4x4dc  = dct4x4dc;// H/W in macroblock.c
    dctf->idct4x4dc = idct4x4dc;// H/W in macroblock.c

    dctf->dct2x4dc = dct2x4dc;// H/W in macroblock.c
...
#define ZIGZAG4_FIELD\
 ...
 void zigzag_scan_8x8_frame( dctcoef level[64], dctcoef dct[64] )
{
    ZIGZAG8_FRAME
}

 void zigzag_scan_8x8_field( dctcoef level[64], dctcoef dct[64] )
{
    ZIGZAG8_FIELD
}
 ...
 void zigzag_scan_4x4_frame( dctcoef level[16], dctcoef dct[16] )
{
 ...
 void zigzag_scan_4x4_field( dctcoef level[16], dctcoef dct[16] )
{
...
#define COPY8x8\
 ...
 int zigzag_sub_4x4_frame( dctcoef level[16], const pixel *p_src, pixel *p_dst )
{
    int nz = 0;
    ZIGZAG4_FRAME
    COPY4x4
 ...
 int zigzag_sub_4x4_field( dctcoef level[16], const pixel *p_src, pixel *p_dst )
{
    int nz = 0;
    ZIGZAG4_FIELD
    COPY4x4
...
#define ZIGDC(i,y,x) {\
 ...
 int zigzag_sub_4x4ac_frame( dctcoef level[16], const pixel *p_src, pixel *p_dst, dctcoef *dc )
{
    int nz = 0;
    ZIGZAG4_FRAME
    COPY4x4
 ...
 int zigzag_sub_4x4ac_field( dctcoef level[16], const pixel *p_src, pixel *p_dst, dctcoef *dc )
{
    int nz = 0;
    ZIGZAG4_FIELD
    COPY4x4
 ...
 int zigzag_sub_8x8_frame( dctcoef level[64], const pixel *p_src, pixel *p_dst )
{
    int nz = 0;
    ZIGZAG8_FRAME
    COPY8x8
 ...
 int zigzag_sub_8x8_field( dctcoef level[64], const pixel *p_src, pixel *p_dst )
{
    int nz = 0;
    ZIGZAG8_FIELD
    COPY8x8
...
#undef COPY4x4
 ...
 void zigzag_interleave_8x8_cavlc( dctcoef *dst, dctcoef *src, uint8_t *nnz )
{
    for( int i = 0; i < 4; i++ )
    {
        int nz = 0;
...
{
    pf_interlaced->scan_8x8   = zigzag_scan_8x8_field;// H/W in macroblock.c and rdo.c
    pf_progressive->scan_8x8  = zigzag_scan_8x8_frame;// H/W in macroblock.c and rdo.c
    pf_interlaced->scan_4x4   = zigzag_scan_4x4_field;// H/W in macroblock.c and rdo.c
    pf_progressive->scan_4x4  = zigzag_scan_4x4_frame;// H/W in macroblock.c and rdo.c
    pf_interlaced->sub_8x8    = zigzag_sub_8x8_field;// H/W in macroblock.c
    pf_progressive->sub_8x8   = zigzag_sub_8x8_frame;// H/W in macroblock.c
    pf_interlaced->sub_4x4    = zigzag_sub_4x4_field;// H/W in macroblock.c 
    pf_progressive->sub_4x4   = zigzag_sub_4x4_frame;// H/W in macroblock.c
    pf_interlaced->sub_4x4ac  = zigzag_sub_4x4ac_field;// H/W in macroblock.c
    pf_progressive->sub_4x4ac = zigzag_sub_4x4ac_frame;// H/W in macroblock.c in our case interlaced will work only.



    pf_interlaced->interleave_8x8_cavlc =
    pf_progressive->interleave_8x8_cavlc = zigzag_interleave_8x8_cavlc;// H/W but cavls is not required
